 ...
 ********************************************************************************/
 int process_command(int* p_to_sockfd,int current_userID,char* p_msg_to_slef,char*p_msg_to_opp,const char*original_msg)
 {
    ...
    else if(original_msg[0]=='1')
    {

    }
    else if(original_msg[0]=='2')
    {

    }
    else if(original_msg[0]=='3')
    {
...
strcat(opp_name,"username:");
for(i=0;i<100&&original_msg[i]!=':';i++)
    opp_name[i+9]=original_msg[i];
opp_name[i+9]=0;
strcat(opp_name,";password:123;");
    if(check_signup(opp_name)==1)                
    {
    ...
    else
    {
        ...
        if((opp_userID=if_user_online(opp_name))!=-1)         //Friend not online
        {
            ...
            *p_msg_to_opp=0;
            ...
            p_offline_msg=fopen(opp_name,"a");
